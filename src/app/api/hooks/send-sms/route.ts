...
import { requireSmsHookSecret } from "@/lib/api-auth";
 ...
 * 仪表盘配置：Authentication → Hooks → Send SMS → HTTPS，URL = https://<domain>/api/hooks/send-sms，
 * Authorization scheme = Bearer，secret 与 SMS_HOOK_SECRET 一致。
 *
 ...
 *
 * 三条不可退让的规则：
 *  1. **fail-closed 验签**（requireSmsHookSecret）：没配 secret 就 503，绝不放行。
...
/** 给"没有对应 action 记录"的发起来源（例如仪表盘手动触发）补一条审计行。 */
async function findPendingAttempt(phoneE164: string) {
  return db.otpAttempt.findFirst({
    where: { phoneE164, status: "REQUESTED", createdAt: { gte: new Date(Date.now() - 10 * 60 * 1000) } },
    orderBy: { createdAt: "desc" },
  });
}

export async function POST(req: NextRequest) {
  const denied = requireSmsHookSecret(req);
...
async function recordAttempt(phoneE164: string, externalId: string | null, status: string, error: string | null) {
  try {
    const pending = await findPendingAttempt(phoneE164);
    if (pending) {
