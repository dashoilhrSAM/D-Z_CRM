...
>
> **⛔ 本表已冻结（2026-09-11）：新改动不要再往这里加行。**
>
> 改成一次改动写一个文件：`docs/changes/YYYY-MM-DD-<简短英文名>.md`，用 `pnpm new:change <名字>` 生成骨架。
> **原因**：两条分支同时开着时，双方都往本表顶部插一行、往 HANDOFF 顶部插一段，改的是**同一批行**，合并必然冲突——
> 已经发生两次，每次都靠「取 main 版本 + 按锚点插回自己的条目」解决，纯属重复劳动且容易抄错。
> 两条分支各建**不同文件**则永远不冲突。下方为冻结前的历史记录。
>
> 有测试盯着这条规矩：`tests/docs-changes.test.ts`。

| 日期 | 改动 | 影响 |
| --- | --- | --- |
| 2026-09-11 | **修复「关掉的内容仍出现在 rider 资讯」**（分支 fix/rider-off-news-leak，owner 报告）：**根因不在 News 页，在它链接过去的那一页**——`/rider/service-history`（底部导航叫 News）**正确过滤了 published:true**，但它「View all」指向的 **`/rider/promotions` 读全部 MarketingAsset、完全没有 published 过滤**。生产实测：22 个素材中 **10 个是关掉的**，News 显示 12、Promotions 显示 22 —— **10 个被关掉的海报一直漏给骑手**。**顺带发现同类第二处泄漏**：四个 rider 页面各自手写「促销是否生效」的一部分（rider/home、rider/promotions、rider/book、rider/service-history），其中 **rider/book 与 rider/service-history 只查 endDate: { gte: now }**——① 从不检查 startDate，导致**尚未开始的促销被提前展示**；② endDate >= now **会把没有结束日期的长期促销整条排除**（而 isPromoActive 认为 endDate 为空是有效的）。**修法（沿用「规则只写一遍」）**：① promotions 页加 published:true；② 四个页面统一改用既有的 isPromoActive（modules/marketing/promo.ts）作为**唯一的「生效」定义**；③ rider/home 原本先取最新 3 条再过滤——若这 3 条都已结束，首页会一条优惠都不显示（其实后面还有生效的），改为取 24 条再过滤。新增 tests/rider-news-visibility.test.ts 6 例（源码级守卫：**rider 下每一处 marketingAsset.findMany 必须带 published**、四个页面必须使用 isPromoActive、不得在 SQL 里过滤 endDate）+ e2e/rider-news-visibility.spec.ts 2 例 | 关掉的海报不再出现在骑手端任何位置；促销窗口由唯一定义判断，未开始的不会提前露出、无结束日期的不再被误删。**验证做了反向测试**：守卫在旧代码上确实失败（报 reads posters without filtering on published）；e2e 第一版第二例因为 seed 里没有未发布海报而**空跑**（打印 unpublished posters in the seed: 0），已改为自行创建再断言。验证：tsc 0 / lint 0 error / vitest **397**（31 文件）/ next build 0；Playwright 2/2。无 schema 改动 |
| 2026-09-11 | **Content Studio 内容可回访 / 导出 / 标记已发布**（分支 feat/content-studio-save）：owner 问「我要如何 save content studio 的 content」。**先查清事实再动手**：内容其实**早已落库**（ContentScript.expandedJson 存 4 平台 caption+hashtags+海报文案、posterUrl 存海报；生产实测 16 条脚本中 1 条 SELECTED 带完整展开+海报），**真正的问题是「回不去」**——Studio 只把当前一次跑的结果放在浏览器内存里，刷新后界面无入口，所以体感是「没保存」。三个缺口：① 没有历史入口（服务端 **GET 接口早就存在且注释写着「for the studio's history panel」，但前端从未调用**）② 没有一键导出（只能逐平台复制+单独下载海报）③ `status: USED` 数据模型预留却**全代码零写入**。**A 回访**：Studio 新增「已生成内容」面板，调用那个没人用的 GET，点一条即还原 caption/hashtags/海报（新增 `load` action 返回 expandedJson）。只列 `expandedAt` 非空的——半成品候选不列，否则面板会变成第二个更差的 Script Bank。**B 导出**：新增纯模块 `src/modules/marketing/content-export.ts`（`buildExportMarkdown` + `exportFilename`），一个按钮把 4 平台 caption（各自带平台标题）+ hashtags + 海报文案 + 海报链接 + 发布提示导出成单个 .md（**客户端 Blob 下载，零新依赖**，不走 zip）。**C 已发布**：新增 `mark-used` action + **ContentScript.usedAt**（双 schema + 迁移 add_content_script_used_at），列表与按钮显示已发布/未发布并可回退。新增 tests/content-export.test.ts 12 例（导出格式是交付契约，逐项钉住：caption 必须在自己的平台标题下、缺失字段不留空标题、日期非法不打印 Invalid Date、文件名可被文件系统接受）+ e2e/content-studio-save.spec.ts 2 例 | 老板现在能在 Studio 里找回、打包带走、并区分已发未发。**实测（e2e 断言下载内容与数据库）**：下载文件名 `dz-content-e2e-saved-content-elak-clutch-slip.md`，文件内含两个平台的 caption + hashtags + 海报文案 + `- Status: not posted yet`；点「标记为已发布」后数据库 status=USED、usedAt 写实，列表即时变为 Posted。**生产 usedAt 列已直连应用**。验证：tsc 0 / lint 0 error / vitest **403**（30 文件）/ next build 0 |
| 2026-09-10 | **结账时可打折：百分比或金额（owner 要求）**（分支 feat/invoice-manual-discount）：**关键决策——手动折扣不写进 `discountSen`**。该字段是**促销归因金额**：`completion.ts` 按预约的促销快照写入，`performance.ts` 读它算 campaign 的 discountCost 与 roiOnDiscount。柜台折扣写进去会让促销 ROI 静默失真，故另开字段。**新增 5 列（双 schema + 迁移 add_invoice_manual_discount）**：`manualDiscountSen`（实际减免）/ `manualDiscountKind`（PERCENT｜AMOUNT）/ `manualDiscountValue`（原输入）/ `manualDiscountReason` / `manualDiscountAt`——kind 与 value 都存，否则事后无法解释这个数字的来源、也做不到二次编辑。**新增 `src/modules/finance/invoice-discount.ts`**（纯函数、唯一算法）：净额 = subtotal − 促销 − 手动（下限 0），总额 = 净额 + 税；百分比以 **subtotal** 为基准并四舍五入到整 sen；金额型上限 = subtotal − 促销；**返回的是「实际减免」而非「请求减免」**（100% 折扣遇上已有促销时只记真正扣掉的部分，否则报表会高估柜台让利）；`parseDiscount` 校验来自浏览器的输入（server action 是公开端点）。**新增 `setInvoiceDiscount` action**：分行隔离（branch 级不得改他行发票，防猜 id 越权）、**已结清一律拒绝**（那是退款不是折扣）、**折扣后总额不得低于已收款**（否则差额是「该退给客户」而不是「打折」）、同步 `PAY_LATER/PENDING` 应收占位金额、**每次写 AuditLog**（谁/类型/原输入/实际减免/理由）。**UI**：收款弹窗内加「不打折｜百分比｜金额」三选 + 数值 + 原因，**用同一个纯函数实时预览**新总额与未结，确认后先落折扣再收款；若折扣把账清零则自动结清（否则会永远挂着一张零余额未结发票）。展示面三处一致：面板、发票列表、打印页 /invoice/[id] 都分别显示促销与手动折扣（`manualDiscountLabel` 只定义一次）。**权限按 owner 决定**：所有柜台都能打、不限额度、靠审计追溯。新增 tests/invoice-discount.test.ts **21 例** + e2e/invoice-discount.spec.ts 2 例 | 柜台能在收款时直接给折扣，且促销 ROI 不被污染。**实测（e2e 断言数据库）**：发票 RM1025 → 10% → manualDiscountSen 10250、total 92250、促销字段不变、状态 PAID、收款恰为 92250、AuditLog 有 `INVOICE_DISCOUNT_APPLIED`。**生产 schema 已直连应用**：5 列全部就位、17 张发票未受影响（默认 0）、站点 200。验证：tsc 0 / lint 0 error / vitest **391**（29 文件）/ next build 0；Playwright 全量 36 通过、4 失败**与本次无关**（rider 预约超时，已在干净 main 复现过） |
| 2026-09-10 | **发票页可就地看明细（counter 不必跳回工单）**（分支 feat/invoice-quick-view）：owner 要求「每张发票给一个下拉看明细，只放工单行」。**数据源选的是发票自己的 `InvoiceItem` 而不是工单的 items/parts**——`completion.ts` 在完工时把已接受的工单行**快照**成 `InvoiceItem`，所以发票行与旁边的 total 永远对得上；若读实时工单，工单事后被改就会与已开票金额不一致（这正是发票该有的语义）。**实现**：`<details>/<summary>` 原生折叠（**零 JS**，页面是 Server Component；用 `group-open:` 切换 ChevronDown 旋转与「View details / Hide details」文案），每张卡片一个，展开显示「描述 · 数量 × 单价 · 行小计」+ 小计/折扣/税/合计（沿用 `pdf.subtotal`/`pdf.discount`/`pdf.tax`/`common.total` 现有词条）。**新增 `src/lib/invoice-lines.ts`**：`InvoiceItem` **没有 createdAt**，数据库无法给出阅读顺序，故在代码里定序为 SERVICE → FEE → APPROVAL → PART（先做后装），同类别内**稳定排序**保留开票顺序；未知类别排最后而不是丢弃。**空明细已处理**：生产 17 张发票里有 1 张没有任何明细行，展开显示「No line items on this invoice.」。新增 tests/invoice-lines.test.ts 8 例（含「读发票而非工单」的源码级守卫）+ e2e/invoice-details.spec.ts 2 例 | 柜台在发票页就能看清明细，不用再去工单页；明细行与总额始终一致。**实测**：e2e 144 张发票全部渲染出折叠控件，展开后可见 3 行明细（subtotal 16500 = total 16500），截图 app-screenshots/invoice-details-open.png 经视觉核对无错位。验证：tsc 0 / lint 0 error / vitest **370**（28 文件）/ next build 0；Playwright 2/2 |
| 2026-09-10 | **修上一个修过头的地方：编辑工单弹窗里机械师下拉「吃掉」客户要求栏**（分支 fix/job-mechanic-picker）：owner 反馈「现在有点太长了，在 edit jobs details 那里会吃到 customer request 的 column」。**根因是我自己上一条引入的**：为让弹出层够宽，我给触发器写了 `sm:w-80`（固定 320px），但该弹窗把 Mileage / Mechanic / Customer Request 放在 `grid sm:grid-cols-3`（每列约 234px）里，固定宽度直接溢出到右边那一栏。**修法（把两件事分开）**：① **触发器只负责填满自己的列**（`w-full`；创建/维修表单在整行里故用 `w-full max-w-sm`），② **弹出层的可读宽度由 popup 的 `min-w-64` 保证**（`SelectContent` 宽度本来就跟 `--anchor-width`，所以触发器不必为了弹层而变宽）。**验证方式**：不查 class、直接**量几何**——断言 trigger 的右边界 ≤ Customer Request 的左边界。实测 756.9 vs 768.7（留 12px 网格间距），弹层宽 256px。**这个断言同时验证了它能抓 bug**：改完忘了 rebuild 时它在旧构建上确实失败（837.1 > 769.7），把我上一条的溢出量了出来。**顺带发现并修掉一个「假测试」**：`e2e/job-mechanic-picker.spec.ts` 第 2 例用 `a[href^="/workshop/jobs/"]` 点第一个链接，而工单列表上第一个这类链接是 **Create Job**（`/workshop/jobs/new`）——它其实一直在创建页上跑、却声称在测编辑页；已改为**按 id 直接查库取真实工单**。验证：tsc 0 / vitest 362 / next build 0；Playwright 2/2；截图 app-screenshots/edit-job-mechanic-fixed.png |
| 2026-09-10 | **指派机械师：分行过滤 + 下拉裁切（owner 报告）**（分支 fix/job-mechanic-picker）：**① 下拉列的人和 action 接受的人不是一套**——`createJob` 把工单建在【登录者所在分行】（org 级回退主店）并拒绝跨行机械师，而下拉用 `scopedStaffWhere(session)` 列【登录者能看到的全部分行】。生产实测 2 分行 13 人：owner 看到 13 个，其中 Testing Branch 的 4 个**选了必然被拒**（Mechanic belongs to a different branch）。根因不是任一处写错，而是**同一条规则写了两遍**。修法：新增 `src/lib/job-branch.ts` 作为唯一定义（`resolveNewJobBranchId` 新建工单归属分行、`staffWhereForBranch` 纯函数、`loadAssignableStaff`），`createJob` 改为调用它，`jobs/new`（工单将归属的分行）与 `jobs/[id]`（**工单已有的 branchId**，与 assignMechanic 的校验同源）改为按它取人。**② 下拉后半段被切掉**：`SelectContent` 宽度 = `w-(--anchor-width)` 跟着触发器走，而触发器是 `w-fit`（宽度=当前选中项「Assign later」），长选项被 `overflow-x-hidden` **硬裁且无省略号**；且 `ItemText` 是 `shrink-0 + whitespace-nowrap`，等于保证被切。修法：`ItemText` 改 `min-w-0 truncate`（**这是共享 primitive，全站 Select 一起受益**）、`min-w-36 → min-w-40`、三个触发器给稳定宽度。**③ 顺带消重**：`MechanicOption` 原本在三个表单里各声明一遍，合并为 `components/workshop/mechanic-option.ts`，并加 `spansBranches`/`mechanicLabel`——单分行列表不再显示「· 分行名」（那个后缀正是压垮宽度的东西）。新增 tests/job-branch.test.ts 8 例（含「createJob 内不得再内联推导分行」的源码级守卫）与 e2e/job-mechanic-picker.spec.ts 2 例 | 下拉里每一个名字现在都真的能被指派，且文字不再被切。**验证方式值得记**：e2e seed 只有 1 个分行、复现不了该 bug，故测试**自己建第二个分行和机械师**；并**实测「反向」**——临时把过滤去掉后测试确实失败（报 offered a mechanic from E2E Other Branch），证明测试不是空跑。验证：tsc 0 / lint 0 error / vitest **362**（27 文件）/ next build 0；Playwright 2/2 通过；截图 app-screenshots/mechanic-picker-fixed.png |
| 2026-09-10 | **服务价目无法编辑 + 日历会静默过期（两个真实缺陷）**（分支 feat/marketing-prod-data）：**① 生产 8 个服务全部 priceSen=null，而 UI 根本无法设置价格**——`actions/settings.ts` 只有 create/toggle/delete，**没有 update**，服务建出来就永远改不了（生产实测：8/8 无价）。后果不只是「显示难看」：`buildFactSheet` 读到 null 就渲染「price not set」，而内容引擎**拒绝编造价格**，于是所有文案都不能报价——整条报价能力被一个缺失的编辑入口关掉。已新增 `updateServiceType`（org 级鉴权 + 名称非空/价格非负校验，`priceSen: undefined` 保持不变、`null` 清空——否则填错的价格永远拿不掉）+ 服务列表行内编辑（复用现有 create 行样式）+ 无价服务数提示带（i18n 三语）。**② 日历会静默过期**：发薪日是按当月滚动生成的，`leadDays` 地平线就是种下去那 18 个月——手动种一次，18 个月后规划器就没东西可推，**和「表建好了但是空的」是同一种静默失败**。已把写入路径从 `scripts/` 提到 `src/modules/marketing/occasion-seed.ts`（脚本与 cron 共用同一份，避免两份「日历该有什么」漂移），新增 **Vercel Cron `/api/cron/marketing-calendar`（每月 1 号 04:00，Bearer CRON_SECRET 鉴权，实测无头 401）**幂等滚动。**顺带简化**：就绪度报告原本用 raw SQL 数开窗数量，改为调用既有的 `isWindowOpen`（纯函数）——既消掉了 Postgres camelCase 列名必须加引号的坑，也让 sqlite 本地能正常显示（此前恒为 unknown）。新增 tests/marketing-seed.test.ts 扩到 13 例（含开窗边界：lead 前一天不算、当天算、多日节点区间内算、区间外不算） | 老板终于能在 UI 里填真实服务价格，内容引擎随之能报价；日历不会再因为没人记得重跑而变空。验证：tsc 0 / lint 0 error / vitest **354**（26 文件）/ next build 0（/workshop/settings 在构建产物中）；cron 无鉴权实测 401；本地 seed 幂等且就绪度显示 windows open now = 1。⚠️ **未能验证的**：设置页的「编辑」按钮点击链路——本地 dev.db 的 staff 账号走 Supabase 登录、无可用凭据，构建级验证通过但未真机点过，需 owner 点一次确认 |
| 2026-09-10 | **营销数据一条命令落地（生产此前是空的）**（分支 feat/marketing-prod-data）：生产 schema 补齐后三张表**全是 0 行**——日历 38 条节点、22 个产品、2 个品牌档案当初只种进了本地 dev.db。失败模式是**静默的**：不报错，只是 Content Studio 打开后「没有节点、没有产品」像坏了一样。**根因不是漏跑，是设计**：四套数据靠四个脚本 + 一个人记住顺序（品牌回填必须在产品导入之后）——这种约定迟早被跳过，于是被跳过了。**做法**：① 四个脚本改为**导出数据与 seed 函数、仅在直接执行时跑 main**（此前 import 即执行，无法被编排），② 新增**唯一入口 `scripts/seed-marketing-data.ts`（`pnpm seed:marketing`）**按正确顺序执行并打印就绪度，③ 新增 `tests/marketing-seed.test.ts` 8 例（含「payday 地平线随日期滚动」——日历干了就是规划器没东西可推，且同样是静默失败）。**顺带修掉两个真实缺陷**：① `import-promo-products.ts` 每次运行都把 `brand` 写回 null（后接的 backfill 让它看起来没事，但单独跑一次会**把 22 个产品全部去品牌**）——DASHOIL 既然已由 owner 确认，就直接写进数据，`apply-dashoil-brand.ts` 只做存量回填；② 就绪度查询里 `startDate` **未加引号**被 Postgres 折叠成 `startdate` 而报错，且被 `.catch()` 吞掉显示为 unknown（**camelCase 列名必须加引号**）。**新踩坑（已写进脚本提示）**：Prisma client 是按单一 schema 生成的，`schema.prisma` 是 sqlite —— 拿 postgres URL 跑 client 会报 “URL must start with the protocol file:”，必须先 `prisma generate --schema prisma/schema.pg.prisma`、跑完再 `prisma generate` 还原；脚本已把这个错误翻译成确切命令 | **生产实测**：38 节点 / 22 产品 / 2 品牌档案全部就位，`rankOccasions` 当天给出 **Hari Malaysia（窗口 OPEN，还有 5 天）**及其马来语角度提示；重复执行 0 新增（幂等）。验证：tsc 0 / lint 0 error / vitest **349**（26 文件）/ build 0 |
| 2026-09-10 | **修复 Vercel 构建 45 分钟超时（生产部署连续失败）**（分支 fix/build-timeout-schema-sync）：owner 报告 PR#12 的部署因 45 分钟超时失败、PR#13 同样卡住。**先用数据定位，不猜**：查 Vercel API 拿到部署时长——PR#11 **1.5 分钟 READY**、PR#12 **45.6 分钟 ERROR**、PR#13 一直 BUILDING；更早 6 次部署全是 **1–2 分钟**。PR#12 唯一的变化就是把 `sync-prod-schema.mjs` 加进 `buildCommand`。本地实测排除构建本身：`next build` **13 秒**、`prisma generate` **1.4 秒**、`migrate diff` 查生产 **2.1 秒**——**所以不是「慢」，是「挂住」**。**根因**：该步骤是整个构建里唯一会连生产库的环节，而 `execFileSync` **默认没有超时**——Vercel 上 `DATABASE_URL` 若是 Supabase 连接池地址（6543/pooler），Prisma 的 migrate 命令需要真实会话、在池化连接上不会报错而是**一直等**，构建于是被平台在 45 分钟处杀掉。**修法（两道防线）**：① **优先 `DIRECT_URL`**——`resolveUrl()` 按 DRIFT_CHECK_URL → DIRECT_URL → DATABASE_URL 取值，并新增 `looksPooled()` 在检测到池化地址时**大声警告**；② **每条 prisma 调用都有硬超时**（默认 120s，`SCHEMA_SYNC_TIMEOUT_MS` 可调，`killSignal: SIGKILL`），**检查阶段超时 = 失败放行（跳过、构建继续，只读检查不该阻塞发版）**，**应用阶段超时 = 快速失败（半应用比停构建更糟）**；另加每步耗时日志。**超时机制已实测**：用 `execFileSync(timeout:3000, killSignal:SIGKILL)` 验证错误形状确实是 `code=ETIMEDOUT`/`signal=SIGKILL` 且 3.0s 整被杀（不靠假设）；再对不可达地址实跑，脚本 0.5s 内放弃并以「skipping」继续构建。新增 tests/schema-sync.test.ts +6 例（resolveUrl 优先级/本地 sqlite 跳过/looksPooled 识别） | 构建再也不会被一个不响应的数据库吃掉 45 分钟：最坏 ~2 分钟且给出可读原因。**2026-09-10 已用直连地址对生产执行过一次**：66→70 表、657→743 列，新增 Occasion/TrendTopic/PromoProduct/BrandProfile + 10 索引 + 4 外键 + ContentScript.angle（可空），112 条语句 **0 条 DROP**，2.2s 完成并复验 in sync；既有数据未变。⚠️ **仍需 owner 做一件事**：在 Vercel 加 `DIRECT_URL`（Supabase 直连 5432，非池化），否则未来改 schema 时构建会跳过应用（不再挂起，但需人工补 DDL）。⚠️ **另一缺口**：生产 Occasion=0 / PromoProduct=0，需在生产跑 `scripts/seed-occasions.ts` 与 `scripts/import-promo-products.ts`。验证：tsc 0 / lint 0 error / vitest **341**（25 文件）/ next build 0 |
| 2026-09-10 | **抠图印刷化处理（owner 采纳建议方案 c）+ 取消产品窗**（同分支 feat/marketing-content-engine）：承接上一条——照片 vs 平面插画的风格差消不掉，owner 说「根据你的建议」，故按方案 c 实施：**把照片本身变成印刷品**。① 新增 `poster-print.ts`：对比度 1.12 + 饱和度 1.06 + **6 级色阶量化** + **斜向网点纹理（13% 不透明度，pitch 随产品尺寸缩放）**。**为什么是 6 级而不是真海报的 3 级**：3 级会让标签上的小字不可读，而品牌标签不是我们能溶解的东西；`tonalRange()` 就是守这条线的度量。② **实测验证标签可读**：同一张 E1300 抠图做「处理前 / 处理后」并排给视觉模型逐个转录——**处理后标签文字全部仍可读出**（DASHOIL / E1300+ / API SP JASO MA2 / SYNTHETIC TECHNOLOGY MOTOR OIL / 4T 10W40 / 1.2L / MAXIMUM PERFORMANCE…），评语「text content is identical… with a textured, stylized appearance」「reads like screen-printed」，仅极细部有轻微损失。③ **修掉一个自己造的严重 bug**：网点层用 `joinChannel` 贴 alpha 时，把一张几乎全透明的网点栅格 `removeAlpha()` 会得到**整块不透明的黑色画布**，于是**每个产品都变成纯黑剪影、标签全毁**（代码看不出问题，只有图能看出来）。改为**手动把两个 alpha 相乘**（一行算术，不可能再犯），并加了「处理后的产品不得变黑/形状要保持/色阶数要下降」的回归测试。④ **顺带取消了「产品窗」**：上一轮为了容纳照片才让模型画摄影窗，现在产品已是印刷品、不再需要容身之处，于是提示词改为**要求预留区与画面连续**（明确禁止 panel/inset/frame/box/plate）、保留精确百分比坐标与站立阴影。**实测评语确认「No distinct panel — the artwork continues through that area」**（上一轮「beige rounded cutout frame」的问题消失）。新增 tests/marketing-poster-print.test.ts 15 例 | 产品与平面海报**不再是「照片贴上去」**：评语从「pasted photograph」变为「part of the printed design」，标签仍然全部可读。验证：tsc 0 / lint 0 error / vitest **335**（25 文件）/ next build 0；真实渲染 treatment 生效、文字读回 ok 无缺失词。**⚠️ 诚实记录未解决项**：产品与背景的**接地感（阴影方向/透视/光照一致性）**仍是评语所指的最大弱点——这需要真正的场景光照理解，或让 AI 画产品（会编造标签，本系统的底线，不做） |
| 2026-09-10 | **产品合成改为「测量化」+ 摄影窗/配色反哺（用户反馈：不要只有字）**（同分支 feat/marketing-content-engine）：艺术方向改成平面后，**设计本身 9/10**（字即构图、无摄影），但**合成真实产品后掉到 6/10**，三次独立评语一致指向同一句「product photo pasted on / the lighting doesn't match」。**先量后改，并推翻了自己一个假设**：① 新增 `poster-grade.ts`——从生成图**采样预留产品窗的实测环境光**（`sampleRegion` 读 `stageRect`，与提示词、合成坐标同一份定义），得出亮度与色偏后对抠图做**部分**校正（亮度最多 ±10%、色偏系数 0.94–1.06），因为产品是品牌自有包装、标签颜色不能被改掉。② **实测否决了「给抠图加墨色描边」**：本以为描边能让照片融入平面海报，两次独立评语都说它「creates a sharp visual separation … making it look like a distinct, pasted-on element」——正是它要治的病，故**撤掉**（源码留注释 + 单测防回退）。③ **真正的解不是改产品，是改海报**：提示词把预留区改为明确的**「摄影窗」**并给出**精确百分比坐标**（SQUARE = left 58% top 26% width 30% height 52%，由 `stageRect` 直接算出、与合成坐标同源），要求窗内只画无缝影棚布景 + 落影、禁止任何物体，且**窗底色必须取自海报自身配色、不得是纯白亮盒**（实测纯白窗在暗色海报上「punches a hole in the composition」）。④ 新增 `product-colours.ts`——**从真实抠图读出包装主色**（跳过透明像素、64×64 降采样、纯函数命名颜色）写进提示词，让模型**围绕产品选色**并声明 PRODUCT PALETTE 优先于风格里写死的配色：标签颜色不能动，能动的是海报配色。新增 tests/marketing-poster-grade.test.ts 15 例 + tests/marketing-product-colours.test.ts 15 例。⑤ **踩到一个只有真实 build 能抓到的坑**：`poster-design.ts` 被客户端组件（Content Studio 的艺术方向选择器）导入，而它又 import 了读图的 `product-colours.ts` → **`sharp` 被打进浏览器包，build 失败**（tsc 与 vitest 全绿、只有 next build 报错，与之前 opentype.js 那次同型）。已把纯色相分析（无 node 依赖）与读图（含 sharp）拆成 `product-colours.ts` / `product-colours-read.ts`，并加**源码级边界单测**（断言该模块不含 sharp/require/node:）防回退 | 产品不再是「随便贴上去」，而是设计里预期的一块摄影区；海报配色由实测包装色驱动。**实测证据**：真实渲染 stage luma 0.484 → brightness 0.900、cast 1.000/1.002/0.983；读回校验 ok 且无缺失词；评语确认「stands in a clearly delineated window, not floating over the flat illustration」。**已知局限（诚实记录）**：照片与平面插画的风格差无法靠光照/调色消除，属固有权衡——可选摄影向风格（CINEMATIC），或接受画册式「产品插图窗」版式。验证：tsc 0 / lint 0 error / vitest **319**（24 文件）/ build 0 |
| 2026-09-10 | **海报改为「艺术方向可选」+ 默认平面海报（用户第二次反馈：不要只有字）**（同分支 feat/marketing-content-engine）：上一条已让模型设计整张海报，但**只有一种视觉方向且偏摄影**，用户反馈「dont focus on the word, it should create a real poster, not only pasting text in」。**做法是先测量再改**：并排实测四种艺术方向，结论明确——**摄影类永远落在「一张图上面压了字」**（原 BOLD/CINEMATIC 得 7/10，评语与用户原话一致："text placed over an image rather than integrated into the design"），而**平面类字即构图的一部分**（FLAT GRAPHIC 9/10，评语 "designed, integrated graphic poster… text woven into the layout"；INDUSTRIAL 8/10；BLUEPRINT 7/10 但把 WhatsApp 渲染成 WhaltsApp，说明文字校验必须保留）。故 **poster-design.ts 重写为艺术方向系统**：`POSTER_STYLES`（GRAPHIC/INDUSTRIAL/BLUEPRINT/BOLD/CINEMATIC，各带 direction/swatch/summary 与 **`medium: "graphic"\|"photo"`** 标注哪类会退化成压字）+ `styleFor()` + **`DEFAULT_POSTER_STYLE = "GRAPHIC"`**（摄影风保留可选但绝不默认）；brief 由 `mood` 改为 `style` + `subject`。**关键不变量（样式切换不得破坏）**：所有方向都强制「留出空白产品台」——允许用圆框/色块/光晕去**框**它，但不许填充；`POSTER_LAYOUTS` 的合成坐标与提示词描述仍取自同一份定义。风格经 render-poster → `/api/marketing/content` → Content Studio **艺术方向选择器**（`data-testid="poster-style-<KEY>"`）贯通，产出结果持久化。tests/marketing-poster-design.test.ts 25 → **34 例**（新增方向切换/默认值/产品台不变量断言） | 海报从「一种摄影风 + 压字」变为**可选的平面设计方向，默认即平面海报**，直接回应「不要只有字」；产品仍为真实抠图、文字仍读回校验。验证：tsc 0 / lint 0 error / vitest **285**（22 文件）/ next build exit 0；三服务 200、未登录 API 401；真实 key 生成实测（默认 GRAPHIC）= 2079KB、style used: GRAPHIC、product: E1300、text check ok:true / missing:none |
| 2026-09-10 | **海报改为 AI 设计 + 文字校验（用户反馈质量不达标）**（同分支 feat/marketing-content-engine）：原做法是「AI 生成照片背景 → 程序叠字」，产出等于**照片压字**而非海报设计。**推翻了一个过时假设**：一直认为图像模型画不好文字，故才手工叠字——实测 `gpt-image-2.5` **能正确渲染文字**（5 段马来语文案全部拼写正确、无多余文字），于是改为**让模型设计整张海报**（排版层级/徽章/角标线条/网格纹理/渐变/按钮形状），实测评估从「photon+字」变为 7/10 且有真实平面设计元素。**唯一不能交给模型的是产品**：让它画瓶子它会编造标签（实测出现 "FULL SYNTHETIC ENGINE OIL" 等虚构文案），故提示词强制「必须留出空白打光区、禁止绘制任何产品」，再由程序把**真实产品抠图**合成进去。**新增 `poster-design.ts`**：`POSTER_LAYOUTS`（三种尺寸的文字区/产品台坐标）+ `buildDesignPrompt()`——**提示词描述的产品台与合成坐标取同一份定义**，否则瓶子会压到标题上。**新增 `poster-verify.ts` + provider `chatVision()`**：模型写文案带来新风险（拼写错误无人察觉，如实测出现过 "Cuti Lancer" 疑似应为 "Lancar"），故生成后**用视觉模型把海报文字读回、与 brief 比对**；拼错会表现为「缺失词」，缺失则**自动重生成一次**并在 UI 告警。新增 tests/marketing-poster-design.test.ts 25 例 | 海报从「照片压字」升级为「AI 设计的成品海报 + 真实产品 + 文字自动校验」。验证：tsc 0 / lint 0 error / vitest 276（22 文件）/ build 0；实测 1:1 与 9:16 均出图、校验 ok:true、产品标签为真实内容（E300+ V2 / API SP JASO MA2 / 4T 10W40）|
| 2026-09-10 | **模型升级（文本 + 图像）**（同分支 feat/marketing-content-engine）：文本 `gpt-4o-mini` → **`gpt-4.1`**，图像 `gpt-image-1` → **`gpt-image-2.5-sunburst`**；`.env` 显式设 `OPENAI_MODEL`/`OPENAI_IMAGE_MODEL`，代码默认值同步更新（新装环境无需额外配置）。**兼容性修复（关键）**：gpt-5 与 o 系推理模型**拒绝 `max_tokens`（需 `max_completion_tokens`）且只接受默认 temperature**——原代码两种都发，换模型会直接报错；`OpenAIProvider.requestBody()` 现按模型名调整参数（`isReasoningModel()` 判定 `^gpt-5|o[1-9]`），旧模型无条件用新参数名（实测 gpt-4o-mini/gpt-4.1 均接受），故一套代码兼容两代。**实测对比（同 brief 4 条候选）**：gpt-4o-mini 5.1s/套话连篇/未提产品；**gpt-4.1 7.3s/马来语最地道（“rajin redah jalan bandar”“JASO MA2 confirm selamat untuk clutch basah”）/产品与规格准确**；gpt-5.6-luna 12.3s/事实最严谨但偏书面。图像：**gpt-image-2.5-sunburst 比 gpt-image-1 快一倍以上**（1:1 从 41.6s→20.0s、9:16 从 52.9s→22.5s），且**原 9:16 的 52.9s 已逼近 maxDuration=60 的上限**，升级同时消除了这个超时风险。新模型全链路实测：候选 6 条 8.7s（5/6 带真实产品）→ 展开 4.4s → 出图 24.6s，总 37.6s。⚠️ **Vercel 需同步设 `OPENAI_MODEL=gpt-4.1` 与 `OPENAI_IMAGE_MODEL=gpt-image-2.5-sunburst`**（不设则走代码默认值，结果相同）。验证：tsc 0 / lint 0 error / vitest 251 / build 0 |
| 2026-09-10 | **内容引擎 P7：接入 UI（Content Studio）**（同分支 feat/marketing-content-engine）：新增 `/workshop/marketing/content` 页面 + `components/workshop/content-studio.tsx`（需求面板 → 候选卡片 → 展开预览 → 海报，五步一体，**全人工：不自动发布**）+ nav 加 Content Studio 项 + 完整 i18n。**架构决策**：慢操作（生成候选/展开/出图）走 **route handler** `/api/marketing/content`（`export const maxDuration = 60`）而非 Server Action——前者动辄 15-60 秒，Server Action 会被 Vercel 超时切断且调用方无从得知；该路由**强制登录鉴权**（这些调用花钱）。ContentScript 新增 `expandedJson/expandedAt/posterUrl/posterRenderedAt`（双 schema + 迁移 add_content_expansion_and_poster），展开结果持久化，刷新不丢。新增 `modules/marketing/render-poster.ts` 做出图编排。**实测修掉一个真实构建失败**：opentype.js v2 的 CJS 构建有 default、ESM 构建只有命名导出，Turbopack 用 ESM → 默认导入导致 build 失败（**tsc 与 vitest 都未能发现，只有真实 build 能抓到**）；已改为兼容两种模块形态。⚠️ **本项目路径含 `&`，bash 中引用路径必须加引号**（否则被当作后台符号）。**端到端 UI 实测通过**：登录 → Content Studio → 生成 6 条候选 → 选一条 → 展开出 4 平台版本 + 10 hashtags + 海报文案 → 点 1:1 出图 1080×1080 并可下载（截图存 app-screenshots/content-studio.png）。验证：tsc 0 / lint 0 error / vitest 251 / build 0；三服务 200，API 未登录返回 401 |
| 2026-09-10 | **内容引擎 P5/P6：展开成稿 + AI 出图**（同分支 feat/marketing-content-engine）：**依赖变更**：新增 `sharp`（栅格合成）与 `opentype.js` + `@types/opentype.js`（文字转矢量路径）。**P5 展开** `src/modules/marketing/expand.ts`：选中脚本 → 多平台 caption（TIKTOK 150 / IG 300 / FB 500 / WhatsApp 400 字上限，模型受约束生成）+ hashtags（马来语/英语/华语混排，去重限 12）+ **海报文案行**（headline≤5 词、sub≤8、其余≤6，强制截断去尾标点）+ 海报场景描述（明确禁止出现文字/logo/产品）+ 发布提示。**P6 出图** `src/modules/marketing/{images,poster,poster-text,poster-fonts}.ts`：`generateBackground` 调 gpt-image-1 只生成**场景**（prompt 硬编码禁止任何文字/logo/产品）；`composePoster` 三层合成 = AI 背景 → **真实产品抠图**（带模糊接触阴影 + 亮度/饱和度调色以匹配场景光照）→ **矢量文字**；另含 `footerBlocks` 必备品牌页脚（店名+电话，否则海报是匿名图）。**关键技术决策（实测得出）**：文字**必须转成 SVG 路径**而非 <text>——① Vercel 运行环境无 fontconfig，<text> 会静默渲染成空白；② 实测 sharp 自带 librsvg **不支持 @font-face**（用一个系统不存在的字体名渲染，加不加 @font-face 墨迹数完全相同）；③ fontconfig 方案不可靠（sharp 加载即初始化 fontconfig，Next.js 模块加载顺序不可控）。故字体以 base64 内嵌 + opentype.js 转路径，**零字体依赖、本地线上完全一致**。字体经子集化从 316KB/575KB 降至 **19.5KB/16.3KB**（Inter ExtraBold + Noto Sans Bold，均 SIL OFL 1.1 可再分发）。新增 tests/marketing-poster.test.ts 33 例 | 完整链路打通：生成候选 → 人选一条 → 展开成稿 → 出海报。**实测成品**：瓶身读到的是真实标签内容（DASHCIL/E1300+/API SP JASO MA2/4T 10W40/1.2L/MAXIMUM PERFORMANCE）——若让 AI 画瓶子必然是乱码，三层设计目的达成。**已知局限**：产品是"棚拍光"而 AI 背景多为暗调，靠调色只能缓解，仍有轻微贴图感。验证：tsc 0 / lint 0 error / vitest 251（21 文件）/ build 0；真实 key 全链路实测（含 gpt-image-1 出图） |
| 2026-09-10 | **内容引擎 P4：候选脚本生成（用户核心需求）**（同分支 feat/marketing-content-engine）：扩展 **ContentScript** 承载候选（双 schema + 迁移 extend_content_script_for_candidates：status/source/batchId/angle/cta/language/brandKey/occasionKey/trendKey/**includeProduct+productSku（产品开关）**/score/reasoning/generatedAt）——候选就是未选中的 ContentScript，故现有 Script Bank 页面零改动即获得候选工作流。新增 `src/modules/marketing/content.ts`：`buildFactSheet` 从 DB 组装**唯一被允许引用的事实**（PromoProduct 真实规格 + ServiceType 真实价格；价格为空时渲染「price not set」而非编造），`generateScriptCandidates` 一次产出 6-12 条**角度各异**的候选（自动拼接品牌调性 + 当前开启的 Occasion 窗口 + 趋势结构摘要 + 事实表），`selectCandidate` 选中一条并自动 rejected 同批其余，`discardBatch` 整批丢弃。**实测**：产品开关 OFF 时 0 条提及产品；ON 时命中的 SKU 全部真实。**修掉一个真实不一致**：模型有时正文写了「Guna DASHOIL…」却把 includeProduct 留 false（脚本提到产品但海报不会配图）——新增纯函数 `reconcileProductFlag` 以文案为准校正标志位，并在 prompt 里明确要求一致；实测修复后 0 处不一致。新增 tests/marketing-content.test.ts 22 例 | 「先列很多 script → 人选一条 → 再展开」的工作流就位，且产品开关按条粒度可控。**数据缺口**：ServiceType 全部 priceSen=null，故内容目前无法报价（AI 正确选择不报而非编造）——建议 owner 补服务价目。验证：tsc 0 / lint 0 error / vitest 209（20 文件）/ build 0；真实 key 实测三轮生成 + 选中/整批流转 |
| 2026-09-10 | **内容引擎 P3：爆点/爆款结构分析**（同分支 feat/marketing-content-engine）：新增 **TrendTopic 模型**（双 schema + 迁移 add_trend_topic）+ `src/modules/marketing/trends.ts`。**关键约束（决定了做法）**：YouTube 的 `captions.download` 需 OAuth 且仅视频所有者可用 → **拿不到别人爆款的逐字稿**；TikTok 无公开趋势 API（Research API 仅学术）、IG Graph API 仅自有内容 → 所以本模块做的是**结构分析而非逐句拆解**（提取 hookPattern/format/whyItWorks/borrowableAngles），这同时也是合规选择——抄爆款文案是版权问题，学「钩子用的是不作为代价式提问」是正当可复用的。**用手动贴链接即可跑通，不需要 YouTube API key**：oEmbed（YouTube/TikTok 公开无需密钥）取标题作者，OpenAI 做结构提取（strict chatJson）。**相关度守卫已实测**：摩车内容得 5 分并提取出 before/after 钩子，而知名无关爆款（1.5B 播放）仅得 1 分并明确说明原因；`structureDigest` 只把 relevance≥3 且有 hookPattern 的条目喂给后续脚本生成，实测低分条目被自动排除。幂等按 dedupeKey（链接去查询串/尾斜杠/大小写，无链接则回退标题）。新增 tests/marketing-trends.test.ts 19 例 | 爆点能力就位且不依赖付费第三方数据；**待补 YouTube Data API key**（可选，用于自动拉取 MY 地区热门 + 播放/点赞量；无 key 时手动贴链接全流程可用）。验证：tsc 0 / lint 0 error / vitest 187（19 文件）/ build 0；真实 key 实测三案例（oEmbed 解析 / 相关内容 r5 / 无关内容 r1） |
| 2026-09-10 | **内容引擎 P1 收尾 + P2 日历**（同分支 feat/marketing-content-engine）：① 品牌确认 **DASHOIL**（owner 答复）——`scripts/apply-dashoil-brand.ts` 回填 22 个 PromoProduct.brand 并新建 DASHOIL 品牌档案（与 DZ_WORKSHOP 并存）；此前刻意留空是因为 OCR 在 DASHCIL/DASHOIL 间不可靠（连 EF 都被读成 CASTROL，属模式匹配而非识字）。② 新增 **Occasion 模型**（双 schema + 迁移 add_occasion_calendar）+ `src/modules/marketing/occasions.ts`：**核心设计是「内容窗口在日期之前打开」**——每个节点带 `leadDays`（提前多少天开始推）与 `angleHint`（该说什么），因为修车行不是靠节日卖货，而是靠**长假前几百万人要长途骑行回家、想先检查车**这件事卖货；只知节日日期会在节日当天发祝福而错过全部营收窗口。`rankOccasions` 用「相关度 × 窗口内临近度」排序：窗口未开/已过的一律沉底。③ 数据源为官方口径：2026 全国公共假期 15 条 + 2027 年 13 条 + 教育部学校假期，另加季风季（11–3 月，骑行安全内容窗口）与月度发薪日（相关度 2，刻意压低以便让位给真节点）。种入 38 条（FESTIVAL 12 / NATIONAL 5 / MONSOON 2 / SCHOOL_HOLIDAY 1 / PAYDAY 18）。④ 新增 tests/marketing-occasions.test.ts 23 例 | 编排有了「何时该说什么」的数据基础：实测 2026-09-10 当天唯 Hari Malaysia 窗口开启（D-6），系统自动交出主推角度；Raya 给 28 天提前量、CNY 21 天（全年两大出行高峰）。验证：tsc 0 / lint 0 error / vitest 168（18 文件）/ build 0 |
| 2026-09-10 | **Marketing 内容引擎 P1 地基**（分支 feat/marketing-content-engine，从 fix/prod-schema-drift-guard 开出）：**① AI 严格模式 + 结构化输出** src/providers/types.ts 新增 `AiError`/`AiChatOptions.strict`/`AiProvider.chatJson<T>`；openai.ts 实现 strict（缺 key/HTTP 错/空回复一律抛错，不再静默兜底）+ chatJson（自动剥离 markdown 围栏与前后散文、解析失败带原因重试一次）；mock-ai.ts 的 chatJson **刻意抛错**而非编造结构（否则会复现静默污染）。已用真实 key 实测返回合法马来语 JSON。**② PromoProduct 模型**（宣传专用目录，**刻意与库存 Product 分离**——无成本/库存/供应商字段，加产品不会影响库存与财务）+ BrandProfile 模型（品牌调性/do&donts/范例/多语言），双 schema + 迁移 add_promo_product_and_brand_profile。**③ 产品线导入**：22 张自有品牌油品图（原本 86MB/2250×3000）优化为 **WebP 1000px 共 2.4MB**（PNG 需 11.5MB，小 8 倍），落 public/products/<SKU>.webp；用视觉模型从标签**逐个提取真实规格**（黏度/API/JASO/容量/claims/奖项）导入 22 条 PromoProduct。**④ 品牌调性种子** scripts/seed-brand-profiles.ts 写入 DZ_WORKSHOP（马来语优先、语气/受众/do&donts/范例）。新增 tests/ai-strict-json.test.ts 15 例 | 内容引擎有了「不会静默产出垃圾」的 AI 地基与真实产品事实源。⚠️ 待办：产品品牌名未能从图确认（OCR 在 DASHCIL/DASHOIL 间不可靠，已刻意留空而非猜测），需 owner 确认后再补 BrandProfile 与 PromoProduct.brand。验证：tsc 0 / lint 0 error / vitest 145（17 文件）/ build 0；真实 OpenAI key 实测 chatJson 通过 |
| 2026-09-10 | **生产 schema 自动化同步（根治）**（同分支 fix/prod-schema-drift-guard）：把上一条的「只读守卫」升级为 `scripts/sync-prod-schema.mjs`——`VERCEL_ENV=production` 时**检查→应用新增变更→复验**（走 `prisma migrate diff` + `prisma db push --skip-generate`，不带 --accept-data-loss），preview/本地**只检查不改动**。安全设计：① 出现 DROP/TRUNCATE **一律拒绝并 exit 1**（人工决定），② `prisma db push` 自身也会拒绝数据丢失，③ 只碰 `DATABASE_URL` 实际使用的库，本地 sqlite 直接跳过（显式检查用 `DRIFT_CHECK_URL`），④ 库不可达只警告放行，⑤ 新增列向后兼容，先于新代码生效无风险。**关键发现（migrate diff 反向检查）**：生产库存在 schema 里没有的列 `Organisation.qrEnabled`（全代码零引用、git 历史从未出现过、值仅默认 true，是早期手工 DDL 遗留）——若直接接入 db push 会**每轮构建都 DROP 生产列**；已用 `@ignore` 标记（不删列、不进 client、不参与 diff）非破坏性化解，待日后专门清理。同时补上生产真正缺失的 `Booking_servicePackageId_fkey` 外键（已验零孤儿引用后应用）。vercel.json buildCommand 改用 sync 脚本；删除已被取代的 check-prod-schema-drift.mjs 及其测试，新增 tests/schema-sync.test.ts | 合并后**改 schema 只需改 schema.pg.prisma**，构建期自动同步生产，不再依赖人肉记着加列——4 次事故的根因被消除。安全网：破坏性变更仍然拦住，不会误删数据。验证：只读检查对生产=待加 FK；`VERCEL_ENV=production` 实跑=应用成功并复验 in sync；破坏性负向测试（临时 schema 删列）=打印 DROP 并 exit 1；tsc 0 / lint 0error / vitest 130（16 文件）/ build 0；生产 /  /login /contact 全 200 |
