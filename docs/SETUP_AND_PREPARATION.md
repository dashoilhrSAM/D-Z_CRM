...
| 日期 | 改动 | 影响 |
| --- | --- | --- |
| 2026-09-07 | 产品目录管理（feat/product-management）：新增 src/actions/products.ts（createProduct/updateProduct/deleteProduct(软删 active=false)/setProductActive，均 org 级鉴权 + SKU 唯一校验）；产品页 /workshop/inventory/products 由只读改为 ProductManager 客户端组件（org 级可增/改/软删/启停，branch 级只读）；i18n 增 ws.products.* 管理键。产品页 module=PARTS 访问仍是 OWNER+COUNTER_STAFF。 | 可新增/编辑/归档产品目录（org 级共享）；软删保留工单/PO/库存引用；build0/tsc0/test48 全绿 |
