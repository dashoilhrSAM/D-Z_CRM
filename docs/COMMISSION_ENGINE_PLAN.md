...
**已定（2026-09-23，老板按建议采纳）**：

- **A 计佣基数 = 客户实付**：发票层折扣**按行分摊**后作为基数。
  理由：老板已经选了"成交价"，而成交价的字面口径就是客户实际付的钱；若按折扣前算，
  一次 50% 促销会让佣金占收入的比例翻倍。**留一个逃生口**：Organisation 上加布尔开关
  commissionOnGross（默认 false）——若日后发现技师因促销而不满，业务方在界面上就能切，
  而不是让我们改代码（与本项目"营收相关开关放 Organisation 字段"的既有约定一致）。
- **B 未领取奖励 → 窗口结束后自动补发**：计入下一期结算，面板标注"上期补发（自动）"。
  理由：达成是既成事实，不该因为"忘了点按钮"少钱——那会产生无法解释的工资差异。
- **C 存量工单不重算**：历史保留人工值，报告里可查"哪些历史行无法按新规则归因"。
- **D 分行口径：暂不做分行切分，但台账照记 branchId**——将来要按分行统计/结算只是报表层的事，
  不需要改结构（这也是把 branchId 放进台账的原因之一）。
