...
> 状态：**待拍板**（第 10 节有 8 个必须由业务决定的点）。本文件是设计稿，不含实现。
...
model CommissionRule {
  ...
  basis          String   // PERCENT | FIXED   ← 枚举，单值
  value          Int      // PERCENT → 百分点×100（5.25% = 525）；FIXED → sen
...
**计算基数**（必须定死，见问题 2）：lineTotalSen（成交价）vs 折后净额。
推荐**折后净额按行分摊**：促销/优惠发生在发票层，需按行金额比例把 discountSen 分摊到行，
否则"打折的单子佣金照原价算"，老板会立刻发现。分摊算法要纯函数 + 单测（尾差归到最大行，保证合计一致）。
...
- rewardKind 四种（"卖 10 送 2"的三种可能落法，见问题 5）：
...
## 10. 需要你拍板的 8 件事（不定这些，代码没法写）

1. **谁拿钱**：只有被指派的技师？工头要不要抽成（固定 RM 还是 %）？一单多技师怎么分？
   注意：VALID_ROLES 里**没有 FOREMAN 角色**——请定义"工头"是哪个角色 / 如何识别。
2. **计佣基数**：挂牌价、成交价、还是**折后净额**？促销与优惠券是否先扣？（会与现有促销引擎交叉）
3. **计提时点**：工单完工，还是发票收全款？（推荐完工）
4. **免费/保修行**：不计佣金 —— 但**算不算阶梯件数**？
5. **"卖 10 送 2"的 2 是哪种**：一次性 RM 奖励 / 2 件的佣金 / 客户免费 2 件（佣金照给）？
6. **阶梯窗口**：自然月（MYT）/ 结算周期 / 滚动 30 天？
7. **阶梯范围**：按单个 SKU、按分类、还是全店合计？
8. **要不要"组合规则"**（如 5% + 每件 RM2）？还是每层只允许一种算法（推荐，最好解释）？

另两个商业口径问题：

- 存量工单是否重算？（推荐**不重算**，历史保留人工值并在报告里可查）
- 佣金是否要**按分行**统计与结算？（branch-scope 已有基础设施，但 User.branchId 与工单分行要一致）
